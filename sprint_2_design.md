...
- Introduce CLI helper `scripts/stream-run-logs.sh` that follows an existing workflow run (identified reusing `scripts/trigger-and-track.sh` output fed through stdin).
- The helper will:
