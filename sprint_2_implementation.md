...
- Extended `scripts/trigger-and-track.sh` with `--store-dir` to persist run metadata per correlation, enabling later streaming without rerunning correlation.
...
scripts/trigger-and-track.sh --webhook-url "$WEBHOOK_URL" --store-dir runs \
...
# Trigger long-running logger on demand
...
scripts/test-trigger-and-track.sh --webhook-url "$WEBHOOK_URL" --store-dir runs \
  | scripts/stream-run-logs.sh --interval 2
```
